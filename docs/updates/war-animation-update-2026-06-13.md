...
- 当前地图合同为 `real-terrain-texture-runtime-relief-contours-no-polygon-blocks`，已废弃全图天气罩、SVG wash/sheen 伪 3D 和 `typed-regional-palette-v2` 大面积 polygon fill 色块。
...
- The accepted map contract is `real-terrain-texture-runtime-relief-contours-no-polygon-blocks`; full-map weather veils, SVG pseudo-3D wash/sheen layers, and `typed-regional-palette-v2` polygon fill blocks are rejected.
...
- 地图不再靠三块高透明度大面色层调色；随后尝试的六个 MapLibre 低透明度地形角色 fill 层被用户截图判定为大色块问题，已废弃。当前合同改为 `data-terrain-color-model="real-terrain-texture-runtime-relief-contours-no-polygon-blocks"`，`data-visual-surface-contract="maplibre-real-terrain-no-polygon-color-blocks"`，专项门禁检查这些被禁用的大色块 layer ID 不得出现在实际 MapLibre style 中。地图质感回到真实 topo/DEM/hillshade、runtime relief/contours、最终截图色彩评分、项目自有地名标签和局部天气作战单位。
...
- 2026-06-15 第五层 GIS 验证版继续把 committed Terrarium tiles 派生为 runtime contour GeoJSON 和轻量 relief texture，并接入 MapLibre underlay。最终证据 `artifacts/london-air-terrain-gis-runtime-20260615-final-v6/metrics.browser.json` 记录 bundle `/assets/index-DMruXwiJ.js`、CSS `/assets/index-Dheb11PY.css`，`consoleErrors=[]`、`pageErrors=[]`，六帧 `daylightColorGate` 全 true，`cyanGreenSeaRatio≈0.162-0.178`；runtime contour 三层和 `battle-of-britain-gis-relief-texture` 均加载，开场/上午可见云朵 `5` 个、午后 `4` 个、海峡追击 `3` 个；密集段与第三小时播放段保持 `uniqueCameraTransforms=1`、`uniqueMapCenters=1`、`uniqueMapZooms=1`、`terrainCanvasRectMaxDelta=0`。
