...
- 同轮新增 ComfyUI 位图天气资产链：`scripts/run-comfyui-britain-weather-assets.py` 复用 `/Users/asukarei/Documents/我心飞翔/tools/ComfyUI` 现有 SD1.5 服务生成云体 RGB，项目脚本再做受控 alpha/distribution mask，正式运行资产为 `public/assets/weather/battle-of-britain/morning-cloud-bank.png` 与 `afternoon-cloud-breaks.png`，版本参数 `?v=20260614-comfy-weather-v1`。证据 `artifacts/london-air-comfy-weather-20260614-v5/manifest.json` 记录 `failures={}`；上午云层 `alphaRatio≈0.081`、`opaqueRatio≈0.365`，下午云层 `alphaRatio≈0.041`、`opaqueRatio≈0.274`，两者 `edgeVisibleRatio=0`，用于保证云层有天气质感但不是矩形雾毯。
