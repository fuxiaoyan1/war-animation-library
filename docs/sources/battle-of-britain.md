...
- 2026-06-14 用户继续指出中段仍有抖动、海面过绿、地图太浅、地名不可读后，本轮改为先定配色体系再调参：海面为低绿偏钢蓝，陆地为暖土/橄榄辅色，作战单位和航线保持第一视觉层；Esri topo raster 降为纹理参考，`data-topo-raster-opacity="0.24"` 且 `data-topo-labels-suppressed="true"`，避免第三方英文底图标注和黑色碎线主导画面。`BattleOfBritainTerrain3D` 合法化 MapLibre raster 参数并提高 hillshade 到 `0.82`，MapLibre 相机同步阈值提高到 `0.025-zoom`，避免密集航线阶段无意义重同步。
- 同轮新增通用地图配色与明暗评分工艺文档 `docs/tools/map-color-grading-workflow.md`。本片最终浏览器证据保存在 `artifacts/london-air-palette-jitter-fix-20260614-v4/`：运行 bundle `/assets/index-CxdD5iBE.js`，CSS `/assets/index-CrUb0PkW.css`，六个关键帧明暗度评分 `brightnessScore100≈50.78-52.06`，饱和度均值约 `47.40-49.35`，钢蓝比例约 `0.425-0.519`，绿色主导比例约 `0.005-0.017`，`daylightColorGate` 全项通过。上午和下午密集段连续帧记录 `uniqueCameraTransforms=1`、`uniqueMapCenters=1`、`uniqueMapZooms=1`、`stageRectMaxDelta=0`、`terrainCanvasRectMaxDelta=0`，说明相机、地形 canvas 与战术层没有回到用户截图中的抖动状态。截图只保存在 artifacts，不在会话中展示。
