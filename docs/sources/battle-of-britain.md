...
- 2026-06-15 用户截图确认六个 MapLibre 分区 fill 层形成大色块，按产品视觉判定废弃 `typed-regional-palette-v2`。`BattleOfBritainTerrain3D` 删除 `battle-of-britain-channel-color`、`battle-of-britain-channel-lane-color`、`battle-of-britain-england-downs-color`、`battle-of-britain-thames-lowland-color`、`battle-of-britain-france-chalk-color`、`battle-of-britain-france-inland-color` 和 `battle-of-britain-palette-boundaries`，运行合同改为 `real-terrain-texture-no-polygon-blocks`。伦敦专项 smoke 和视觉证据脚本检查这些 layer ID 不得出现在实际 MapLibre style 中，并新增 12:30 第三小时播放段连续帧探针，专门覆盖用户指出的第三小时附近抖动。
- 2026-06-15 接手后继续修正废弃 polygon fill 后的偏土绿问题：MapLibre canvas 使用 `saturate(2) contrast(1.16) brightness(0.71) hue-rotate(14deg)`，topo raster opacity 为 `0.15`，保留真实 topo/DEM/hillshade 与项目自有地名，不恢复六层大色块。证据 `artifacts/london-air-followup-20260615-v2/metrics.browser.json` 记录六个关键帧 `daylightColorGate` 全项通过，钢蓝比例约 `0.306-0.584`、绿色主导比例约 `0.070-0.103`，且第三小时回程走廊连续帧 `uniqueCameraTransforms=1`、`uniqueMapCenters=1`、`uniqueMapZooms=1`。
