...
- `public/assets/unit-icons/trafalgar-hms-victory.webp`
- `public/assets/unit-icons/trafalgar-royal-sovereign.webp`
- `public/assets/unit-icons/trafalgar-bucentaure.webp`
- `public/assets/unit-icons/trafalgar-santisima-trinidad.webp`
- `public/assets/unit-icons/trafalgar-british-line.webp`
- `public/assets/unit-icons/trafalgar-french-line.webp`
- `public/assets/unit-icons/ww2-transport-ship.webp`
- `public/assets/unit-icons/ww2-escort-ship.webp`
- `public/assets/unit-icons/ww2-submarine.webp`
...
Baidu Images, PNG素材网, PNGIMG, Wikimedia Commons, project-game art repositories, museum/library archives, and similar image sites may be used to discover candidate unit references. A candidate becomes a project asset only after the original page or image URL, local source-file path, license status or uncertainty, intended usage scope, and repeatable processing step are recorded here or in the relevant `docs/sources/<animation>.md`.

When license status is unclear, the processed marker is limited to local/private non-commercial demo use and must be replaced with public-domain, permissively licensed, purchased, or otherwise authorized assets before public or commercial distribution. Do not hotlink search-result URLs or overwrite runtime markers with untracked downloads.

For local/private demo production, license uncertainty is a documentation and release-risk item, not a reason to keep bad placeholders. If a tracked candidate source produces a clearer, era-specific, faction-distinct marker, use it for the local demo, record the uncertainty, and keep the public-release replacement requirement visible.

## 候选素材发现渠道

百度图片、PNG素材网、PNGIMG、Wikimedia Commons、项目/游戏美术仓库、博物馆/图书馆档案和类似图片站点都可以用于发现候选作战单位参考。候选素材只有在记录原始页面或图片 URL、本地源图路径、许可状态或不确定性、使用范围和可复跑加工步骤后，才能成为项目资产。

许可状态不清的素材仅限本地/私有非商业演示；公开或商业发布前必须替换为公有领域、宽松许可、已购买或其他授权明确的素材。不得热链搜索结果 URL，也不得用未追踪下载文件直接覆盖运行时 marker。

本地/私有演示制作阶段，授权不确定是文档和发布风险事项，不是继续保留难看占位图的理由。若可追踪候选源图能产出更清晰、更符合时代、阵营更易区分的 marker，可以用于本地演示，但必须记录不确定性，并保留公开发布前替换的要求。
...
- Trafalgar age-of-sail markers: six 900 x 360 transparent WebP assets are generated by `scripts/generate-trafalgar-ship-assets.mjs` from local SVG ship drawings under `public/assets/unit-icons/source/trafalgar/`. Earlier Wikimedia/RMG/model-source images are retained in that folder for visual reference and source traceability, but the map no longer uses those crops because several looked like rectangular photos, deck close-ups, or plans at marker scale. The animation still exposes separate assets for `trafalgar-hms-victory.webp`, `trafalgar-royal-sovereign.webp`, `trafalgar-bucentaure.webp`, `trafalgar-santisima-trinidad.webp`, `trafalgar-british-line.webp`, and `trafalgar-french-line.webp`; they now share one coherent side-view silhouette style, with British/French/Spanish color differences and flagship-specific labels/highlights.
- WWII transport, escort, and submarine markers: `public/assets/unit-icons/ww2-transport-ship.webp`, `public/assets/unit-icons/ww2-escort-ship.webp`, and `public/assets/unit-icons/ww2-submarine.webp` are generated by `scripts/generate-ww2-ship-assets.mjs`. The script emits temporary local SVG drawings under ignored `public/assets/unit-icons/source/ww2-ships/` and converts them to transparent WebP markers. They are project-authored side-view markers designed for small map scale; the submarine marker is used by `HX 229 / SC 122：大西洋狼群战` so U 艇 does not fall back to an abstract oval or generic ship.
...
- Cannae Roman/Carthaginian markers: `public/assets/unit-icons/cannae-roman-legion.webp`, `cannae-roman-cavalry.webp`, `cannae-carthaginian-infantry.webp`, `cannae-african-infantry.webp`, `cannae-carthaginian-cavalry.webp`, `cannae-numidian-cavalry.webp`, `cannae-hannibal-command.webp`, and `cannae-paullus-command.webp` are generated by `scripts/generate-cannae-unit-assets.mjs` from Baidu-discovered source images under `public/assets/unit-icons/source/cannae-baidu/`. Current source files are `baidu-roman-warrior-3d.jpeg`, `baidu-roman-mounted-soldier.jpeg`, `baidu-ancient-european-soldier.jpeg`, `baidu-numidian-cavalry-history.jpeg`, and `baidu-ancient-cavalry-lancer.jpeg`. Direct image URLs are recorded in `docs/sources/cannae-battle.md` update notes and the local source filenames preserve the discovery channel. The script crops, tints, outlines, shadows, badges, and exports transparent WebP markers. The Baidu candidate image license status is not verified; these Cannae markers are for local/private non-commercial demo use only and must be replaced with licensed/public-domain/purchased assets or otherwise authorized before public release. The earlier 0 A.D. portrait chain, Theodore Ayrault Dodge plates, and VHV/PNGIMG source-site attempt are rejected for Cannae runtime use and retained only as local trace material if present.
...
When building new battle-specific assets, control black-silhouette failures in the production prompt and generator first: require era references, material color, shield/weapon/horse or body detail, side/faction differences, readable map-scale geometry, and transparent backgrounds. Pixel gates should catch only obvious placeholders, tiny files, broken alpha, same-looking factions, or near-black regressions; they must not become the main design driver.
