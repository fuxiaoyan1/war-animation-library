...
> 2026-05-26 更新：新会话请优先阅读 `docs/war-animation-handoff-20260526.md`。本文件保留为历史经验库，其中 2026-05-23 的“当前未提交大量变更”描述已过期；当前提交基线和工作区状态以新版交接文档为准。
...
- 鱼雷/深弹效果和舰船航迹对不上，或看起来像舰炮/空战集火。夜间鱼雷 `salvo` 不应使用固定旧坐标跨长时间播放，必须用 `toRouteId` 绑定当时 HX 229 / SC 122 船队航迹；U-384 深弹效果用 `toRouteId: "u384-continuous-track"`、`fromRouteId: "u384-hunt-by-air"` 绑定实时位置。潜艇战不使用集火攻击线，相关效果统一 `showShellTraces: false`，只保留局部命中/水下爆震。
- 发现/合围节点乱放爆炸音。当前只有夜间鱼雷、第二夜攻击和 U-384 深弹节点有战斗音。
- 时间轴拖动或事件跳转后，上一事件的延迟音效不能滞后响到新节点；但不能粗暴取消同一次 `combined` 命中的爆炸延迟，否则鱼雷/深弹只剩前段声。当前做法是事件跳转清理所有待触发 cue，普通时间轴拖动只清理飞机/机枪延迟尾音，保留爆炸延迟。
