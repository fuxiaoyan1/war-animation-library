...
- 鱼雷/深弹效果和舰船航迹对不上。夜间鱼雷 `salvo` 不应使用固定旧坐标跨长时间播放，必须用 `toRouteId` 绑定当时 HX 229 / SC 122 船队航迹；U-384 深弹效果用 `toRouteId: "u384-continuous-track"`、`fromRouteId: "u384-hunt-by-air"` 绑定实时位置，避免船队或潜艇已经脱离但爆点仍在旧点位。
